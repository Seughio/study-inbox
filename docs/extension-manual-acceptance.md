...
DeepSeekAdapter，第 9 节用于 Milestone 2C-1 最小 DoubaoAdapter 的真实 Edge 人工验收；
只使用全新合成测试问答，不打开真实历史资料。
...
## 9. Milestone 2C-1：豆包 Edge 真实页面验收

仅使用 `https://www.doubao.com/chat/<conversation-id>` 普通对话页和纯文字合成内容。不要
启用深度思考、联网搜索、引用、重新生成、编辑、停止、图片、语音、文件、代码、表格、
智能体或豆包应用；这些结构必须安全失败。

### 9.1 构建、授权与单轮

1. 在仓库根目录运行 `scripts\build-extension.ps1`，在 `edge://extensions` 重新加载
   `apps\extension\dist`。
2. 启动独立 FastAPI 数据目录，例如
   `.\scripts\start.ps1 -DataDirectory .\.milestone-2c1-data`。
3. 打开一份全新合成豆包普通对话，展开 popup 的“豆包侦察”，启用开发模式并点击
   “主动授权豆包页面”；授权后可以关闭侦察开发模式，正式采集不依赖侦察状态。
4. 刷新普通对话页，使授权后动态注册的 content script 加载。确认首页、带 hash 的 URL
   和非 `/chat/<id>` 页面不会采集。
5. 记录 API 数量，提出一条普通纯文字合成问题。流式生成期间反复查询，确认不入库；
   `data-streaming` 结束且正文稳定至少 2 秒后，确认只新增一条。
6. 核对问题和最终回答不含复制、点赞、建议项等操作区内容；刷新页面并等待稳定窗口，
   确认相同 `event_id` 不会重复写入。

### 9.2 三轮、暂停与新内容

1. 新建合成对话，连续完成三轮内容明显不同的普通文字问答，每轮等待回答完成。
2. 查询 API，确认按页面顺序新增三条，每个助手回答只配对其之前最近的用户消息；刷新后
   三条均不重复。
3. 关闭 popup 主开关，在暂停期间完成一轮问答；恢复开关后不操作并等待，确认该轮永不
   补录。刷新页面、重新加载扩展后再次确认不补录。
4. 再次暂停并开始一条流式回答，正文出现后恢复采集，让回答自然完成，确认该轮仍不入库
   或 RetryQueue。
5. 恢复后提出全新问题，确认它完成后正常新增一次。

### 9.3 离线、alarm、Markdown 与撤权

1. 停止 FastAPI，保持采集开启并完成一条全新普通文字问答；确认 popup 显示 `queued`，
   RetryQueue 增加且 `study-inbox-retry-queue` alarm 存在。
2. 恢复相同数据目录的 FastAPI；不要刷新豆包页面、不要打开 popup、不要进行用户操作。
3. 在正常浏览器调度下等待最多 1 分钟，确认队列事件自动入库一次、队列清空、alarm 取消，
   证明是 Service Worker 自动恢复投递而不是 DOM 重扫。
4. 调用 Markdown 导出，确认文件中问题与最终回答正确且没有操作区内容。
5. 展开豆包侦察面板并点击“撤销豆包页面权限”，再刷新或提出新问题，确认正式 content
   script 已取消注册且不再采集；DeepSeek 权限和状态不受影响。

### 9.4 验收记录

记录 Edge 版本、构建 commit、授权/撤销状态、单轮和三轮新增数、流式期间新增数、刷新后
新增数、暂停/跨恢复/重载结果、离线队列数、alarm 自动恢复耗时和 Markdown 输出。任何
未支持结构被记录都视为失败；安全不记录是预期行为。
