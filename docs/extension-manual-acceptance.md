...
1. 在 popup 取消“启用采集”，再完成一条普通合成问答；等待超过稳定窗口，确认 API 和 RetryQueue 均不变化。
2. 重新启用采集，在 FastAPI 窗口按 `Ctrl+C`，然后完成一条新的普通合成问答。
3. 等待回答稳定后打开 popup，确认本地服务离线且待重试数量增加。
4. 用第 8.1 节命令恢复相同 `.milestone-2b1-data` FastAPI。
5. 刷新 DeepSeek 页面以触发同一稳定历史事件；该次成功投递会同时触发现有 RetryQueue 自动重试。
6. 确认待重试恢复为 0，同一问答在数据库中只有一个 `event_id`，没有重复写入。
