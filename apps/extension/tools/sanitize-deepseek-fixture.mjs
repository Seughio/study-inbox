...
export function sanitizeDeepSeekHtml(rawHtml, options) {
  if (!options?.userSelector || !options?.assistantSelector) {
    throw new Error("必须显式提供已人工确认的用户节点和助手节点选择器。反猜测选择器被禁止。");
  }

  const dom = new JSDOM(rawHtml, { url: "https://chat.deepseek.com/" });
  ...
  userNodes.forEach((node) =>
    replaceTextPreservingStructure(node, SYNTHETIC_USER_TEXT, dom.window)
  );
...
export async function sanitizeFixtureFile({ input, output, userSelector, assistantSelector }) {
  ...
  if (isInside(inputPath, fixtureDirectory)) {
    throw new Error("原始 HTML 不得放入 Git fixture 目录；请从仓库外的临时位置读取。");
  }

  const rawHtml = await readFile(inputPath, "utf8");
  const result = sanitizeDeepSeekHtml(rawHtml, { userSelector, assistantSelector });
...
async function main() {
  ...
  if (!args.input || !args.output || !args["user-selector"] || !args["assistant-selector"]) {
    throw new Error(
      "用法：node tools/sanitize-deepseek-fixture.mjs --input <仓库外原始HTML> --output <fixture> --user-selector <已确认选择器> --assistant-selector <已确认选择器>"
    );
  }
