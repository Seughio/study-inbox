...
Milestone 2B-0.1 提供开发用 DeepSeek DOM 侦察入口和可逐级切换父子节点的高亮选择器。该入口默认关闭，必须由用户在
popup 开启侦察开发模式、授予可选主机权限并点击具体操作；它不会采集 Cookie、
Web Storage 或网络数据，也不会自动保存未脱敏 DOM。当前没有 `DeepSeekAdapter`。
...
`docs/deepseek-selector-report.md`。当前 `fixtures/deepseek` 中的 HTML 均为占位说明，
不能作为适配器测试依据。
